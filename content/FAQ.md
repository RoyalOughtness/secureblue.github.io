...
secureblue is a Linux operating system with known security holes filled and proactive exploit mitigations in place. Virtualization [works on secureblue](#libvirt), is not its focus. secureblue lacks [several virtualization features](https://www.qubes-os.org/doc/) that QubesOS has. secureblue also places emphasis on usability, while QubesOS is considered by many to have a steep learning curve.
